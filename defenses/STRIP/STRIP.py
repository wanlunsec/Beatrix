...
def strip(opt, mode="clean"):
    ...
    ckpt_dir = os.path.join(opt.checkpoints, opt.dataset, opt.attack_mode, 'target_'+str(opt.true_target_label))
    ...
    if mode == "attack":
        ...
        print(f'type(test_dataloader):{type(test_dataloader)}')
        ...
        print(f'bd_inputs.shape: {bd_inputs.shape}')
...
def main():
    ...
    result_path1 = os.path.join(result_dir, opt.attack_mode)
    if not os.path.exists(result_path1):
        os.makedirs(result_path1)
    result_path = os.path.join(result_path1,'target_'+str(opt.true_target_label))
