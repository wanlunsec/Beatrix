...
class option:
    ...
    attack_mode = "all2one"#"badnet_watermark"#"clean" #"badnet"#"badnet_square"#
    true_target_label = 0
    input_channel = 3
...
def _get_classifier(opt):
    ...
    classifier.eval()
    return classifier.to(opt.device), ckpt_path
...
def save_result(opt, result):
    ...
    with open(output_path, "a+") as f:
        f.write(f'max reasr: {str(result[0])}, optz_label: {str(result[1])}, sample_label: {str(result[2])}\n')
...
def check_values(images, labels, model, children, target_layers):
    ...
    for layer_i in range(0, len(children) - 1):
        ...
        # print(f'len(images):{len(images)}//batch_size:{batch_size}')
        max_val = -np.inf
        for i in range(len(images)//batch_size):
            batch_data = torch.FloatTensor(images[batch_size*i:batch_size*(i+1)])
            batch_data = batch_data.to(opt.device)
    ...
    for layer_i in sample_layers:
        ...
        for input_i in range(n_images//batch_size):
            ...
            for nt in range(nbatches):
                ...
                f_h_t_t = torch.FloatTensor(f_h_t).to(opt.device)
...
def loss_fn(inner_outputs_b, inner_outputs_a, logits, con_mask, neuron, tlabel, acc, e, re_epochs):
    neuron_mask = torch.zeros([1, inner_outputs_a.shape[1],1,1]).to(opt.device)
...
def reverse_engineer(model_type, model, children, oimages, olabels, weights_file, Troj_Layer, Troj_Neuron, Troj_Label, Troj_size, re_epochs):
    ...
    # delta = torch.randn(1,3,h,w).to(opt.device)
    delta = torch.rand(1,3,h,w).to(opt.device) * 2 - 1
    mask = filter_img().reshape((1,1,h,w)) * 6 - 4
    mask= torch.FloatTensor(mask).to(opt.device)
...
def main(model_filepath, result_filepath, scratch_dirpath, examples_dirpath, k, example_img_format='png'):
    opt.true_target_label = k
    start = time.time()

    examples_dirpath = os.path.join(examples_dirpath,opt.dataset)
    ...
    if opt.dataset == "cifar10":
        ...
        transformer = torchvision.transforms.Compose([torchvision.transforms.Resize([h,w]),
    ...
    for fn in fns:
        ...
        img_name = os.path.split(fn)[-1]
        fys.append(int(img_name.split('_')[-3]))
        ...
        # img = np.transpose(img, (2, 0, 1)) ## include this in torchvision.transforms
    ...
    #model = torch.load(model_filepath, map_location=opt.device)
    model, model_filepath = _get_classifier(opt)
    ...
    if len(results) > 0:
        ...
        for result in results:
            ...
            result = [reasr, optz_label, samp_label]
            save_result(opt, result)
    ...
    for i in range(len(reasr_info)):
        ...
        if reasr > max_reasr :
            max_reasr = reasr
            optz_target_label = reasr_info[i][2]
    print('{0} {1} {2} {3} {4} {5} {6} {7} {8}'.format(model_filepath, model_type,
                                                       'mask', freasr, 'sampling val', sampling_val,
                                                       'time', sample_end - start, optm_end - sample_end,))
    if max_reasr >= 0.88:
        output = 1 - 1e-1
    else:
        output =     1e-1
    print('max reasr:', max_reasr, 'output:', output, 'optz_target_label:',optz_target_label)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--result_filepath', type=str, help='File path to the file where output result should be written. After execution this file should contain a single line with a single floating point trojan probability.', default='./output/result.txt')
    ...
    for k in range(0,10):
        main(args.model_filepath, args.result_filepath, args.scratch_dirpath, args.examples_dirpath, k)
